...
from policy_compliance_tracker.providers.analysis_providers import (
    PROVIDER_LABELS,
    provider_configuration_message,
    provider_is_configured,
    provider_model,
)
from policy_compliance_tracker.agent.compliance_agent import analyze_regulation
from policy_compliance_tracker.config import REGULATION_DIR, TRACKER_PRIORITY_VALUES, TRACKER_STATUS_VALUES
...
from policy_compliance_tracker.retrieval.rag_eval import evaluate_retrieval_quality
from policy_compliance_tracker.ingestion.regulation_monitor import (
    clean_title,
    detect_regulator,
    extract_pdf_bytes,
    scan_regulation_directory,
)
from policy_compliance_tracker.ingestion.regulatory_feeds import DEFAULT_FEEDS, ingest_feeds
from policy_compliance_tracker.storage.tracker_store import (
...
st.set_page_config(
    page_title="Policy Compliance Tracker",
    page_icon="R",
    layout="wide",
...
def analysis_metadata(regulation_text, source_path="Manual input", regulator_source=None):
    return {
        "title": clean_title(regulation_text, source_path),
        "regulator": detect_regulator(regulation_text, source_path),
        "version": 1,
        "source_path": source_path,
        "regulator_source": regulator_source or source_path,
    }
...
if "analysis_provider" not in st.session_state:
    st.session_state.analysis_provider = "rule_based"
...
<div class="topbar-shell">
  <div class="topbar">
    <div class="title">
      <div class="eyebrow">Legal &amp; Compliance Operations</div>
      <h1>Policy Compliance Tracker</h1>
...
with tab_tracker:
    ...
    with st.container(border=True):
        st.markdown(
            '<div class="section-heading"><h2>Persistent Tracker</h2><span>Filter, update, and export the compliance impact register</span></div>',
            unsafe_allow_html=True,
        )
    ...
    if entries:
        with st.container(border=True):
            ...
            review_label = "Required" if detail_entry.get("review_required") else "Not required"
            st.markdown("##### Review Gate")
            st.write(review_label)
            if detail_entry.get("review_reason"):
...
with tab_analyze:
    ...
    with st.container(border=True):
        st.markdown(
            '<div class="tool-panel-title"><h3>Regulatory Input</h3><span>Paste text, upload a PDF, or start from a sample</span></div>',
            unsafe_allow_html=True,
        )
        input_col, provider_col = st.columns([1, 1], gap="medium")
        with input_col:
            input_mode = st.radio(
                "Input Source",
        ...
        with sample_col:
            sample_name = st.selectbox(
                ...
                ["No sample selected"] + list(SAMPLES.keys()),
        ...
        with sample_action_col:
            ...
            if st.button(
                "Load Sample",
                width="stretch",
                disabled=input_mode != "Paste text" or sample_name == "No sample selected",
            ):
        ...
        if input_mode == "Paste text":
            ...
            uploaded_file = st.file_uploader(
                "Regulatory PDF",
                type=["pdf"],
                help="The PDF is extracted locally and is not saved unless you use the Automation tab's folder workflow.",
                key="regulatory_pdf",
            )
            regulation_text = ""
            if uploaded_file is not None:
                try:
                    regulation_text = extract_pdf_bytes(uploaded_file.getvalue())
                except ValueError as exc:
                    st.error(str(exc))
                else:
                    source_path = f"Uploaded PDF: {uploaded_file.name}"
                    st.success(f"Extracted {len(regulation_text):,} characters from {uploaded_file.name}.")
                    st.text_area(
                        "Extracted Regulation Text",
                        value=regulation_text,
                        height=220,
                        disabled=True,
                    )

        st.markdown(
            '<div class="action-note">The selected engine will identify obligations, map relevant policies and controls, decide whether policy change is required, and assign a remediation owner.</div>',
            unsafe_allow_html=True,
        )
            ...
            with st.spinner("Analyzing regulation and saving tracker item..."):
                try:
                    result = analyze_regulation(
                        regulation_text,
                        regulation_metadata=analysis_metadata(
                            regulation_text,
                            source_path=source_path,
                            regulator_source="Uploaded PDF" if source_path != "Manual input" else "Manual input",
                        ),
                        persist=True,
