...
CONTROL_SOURCES = [
    "data/controls\\Control_Matrix.pdf",
]
...
SOURCE_DISPLAY_NAMES = {
    ...
    "data/controls\\Control_Matrix.pdf": "Control Matrix",
}
...
def control_evidence_lines(controls):
    ...
    return [
        (
            ...
            f"data/controls\\Control_Matrix.pdf, row {control['id']}"
        )
...
def control_matrix_node(state):
    ...
    control_text = get_source_text(CONTROL_SOURCES)
    controls = extract_control_records(control_text)
...
def analyze_regulation(
    ...
    if persist:
        from tracker_store import save_analysis_result
