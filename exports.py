...
def rows_to_xlsx(rows: List[List[Any]]) -> bytes:
    ...
    core = (
        ...
        '<dc:title>Regulatory Impact Tracker</dc:title>'
...
def tracker_entries_to_pdf(entries: Iterable[Dict[str, Any]]) -> bytes:
    ...
    for entry in entries:
        ...
        lines.append("")
    return text_to_pdf("Regulatory Impact Tracker", "\n".join(lines))


def analysis_to_pdf(result: Dict[str, Any]) -> bytes:
    return text_to_pdf("Regulatory Impact Report", analysis_to_text(result))
